  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (userSnapshot.exists) {
        ...
        _favoritesRef =
            FirebaseDatabase.instance.ref('/favorites/$_userId/${_listing.id}');
        _checkIfFavorite();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
              ...
              ? IconButton(
                  ...
                  onPressed: () async {
                    final updatedListing = await Navigator.push<PlantListing>(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            PlantListingUpdateScreen(listing: _listing),
                      ),
                    );
                        ...
                        Text(
                          _listing.createdAt != null
                              ? DateFormat('dd MMMM yyyy')
                                  .format(_listing.createdAt) // Format the date
                              : 'Unknown',
                  ...
                  children: [
                    Expanded(
                      ...
                      onPressed: () {
                        Clipboard.setData(ClipboardData(
                            text: _listing.sellerContact)); // Copy to clipboard
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: Text(
                                  'Seller contact copied: $_listing.sellerContact')),
                        );
                      },
                      icon: const Icon(
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor:
                              _isFavorite ? Colors.red : Colors.lightGreen,
                          padding: const EdgeInsets.symmetric(vertical: 12),
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(10.0),
                          ),
